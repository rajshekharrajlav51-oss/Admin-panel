...
use App\Types\Settings\MapsSettingType;
    ...
    public function store(Request $request): JsonResponse
    {
        try {
            ...
            $method = match ($type) {
                ...
                SettingTypeEnum::MAPS() => MapsSettingType::class,
            ...
            switch ($type) {
                case 'maps':
                    $currentMaps = Setting::where('variable', 'maps')->first()?->value ?? [];
                    if (empty($values['mapplsStaticKey'])) {
                        $values['mapplsStaticKey'] = $currentMaps['mapplsStaticKey']
