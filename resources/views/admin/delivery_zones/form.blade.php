...
@section('admin-content')
    ...
    <div class="page-body">
        <div class="row row-cards">
            <div class="col-12">
                @php
                    $mapProvider = $mapSettings['mapProvider'] ?? 'mappls';
                    $mapplsStaticKey = $mapSettings['mapplsStaticKey'] ?? '';
                    $googleApiKey = $mapSettings['googleMapKey'] ?? $googleApiKey ?? '';
                    $mapKeyMissing = $mapProvider === 'mappls' ? empty($mapplsStaticKey) : empty($googleApiKey);
                @endphp
                @if($mapKeyMissing)
                    <div class="alert alert-danger alert-dismissible" role="alert">
                        ...
                        <div>
                            <h4 class="alert-heading"><a
                                    href="{{route('admin.settings.show', ['setting' => \App\Enums\SettingTypeEnum::MAPS()])}}"
                                    target="_blank">
                                    {{ $mapProvider === 'mappls' ? 'Mappls Static Key is not configured.' : __('messages.google_api_key_not_found') }}
                                </a>
                            </h4>
                        </div>
                    ...
                    <form
                        ...
                        <div class="card-body">
                            ...
                            <div class="place-autocomplete-card" id="place-autocomplete-card">
                                <input type="search" class="form-control" id="delivery-zone-search"
                                       placeholder="Search location">
                                <div class="list-group mt-2 d-none" id="delivery-zone-search-results"></div>
                            </div>
...
@push('script')
    <script>
        window.AdminMapConfig = {
            provider: @json($mapProvider),
            googleMapKey: @json($googleApiKey),
            mapplsStaticKey: @json($mapplsStaticKey),
            defaultCenter: {
                lat: Number(@json($mapSettings['defaultLatitude'] ?? '28.6139')) || 28.6139,
                lng: Number(@json($mapSettings['defaultLongitude'] ?? '77.2090')) || 77.2090
            },
            defaultZoom: Number(@json($mapSettings['defaultZoom'] ?? '13')) || 13,
            mapplsRoutes: {
